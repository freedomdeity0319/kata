<?php
/**
 * a nice wrapper around http://pear.php.net/package/HTTP_Request/
 *
 * @package kataTest
...
class HTTPTester {
  ...
  private function _prepare($url) {
    $this->_HTTPRequest = & new HTTP_Request($url,array(
  ...
  public function getBody() {
    if ($this->_RESPONSEBODY === false) {
      if (($this->_RESPONSECODE!=301) && ($this->_RESPONSECODE!=302)) {
        throw new Exception("No HTML and no redirect while visiting '".$this->_URL."'. Something fundamental with this request failed.");
      }
      return '';
    }
