...
class Model {
	...
	private function getTableName($n = null, $withPrefix = true) {
		...
		if ($withPrefix) {
			if (null !== $n) {
				return $this->quoteName($this->getPrefix() . $n);
			}
			if ($this->useTable) {
				return $this->quoteName($this->getPrefix() . $this->useTable);
			}
			return $this->quoteName($this->getPrefix() . strtolower($name));
		}
	...
	private function getWhereStringHelper(& $id, $tableName) {
		...
		foreach ($id as $name => $value) {
			...
			//trhow Exception on key is numeric. Since we continued on value or || and, numeric key is only valid for new sub-condition=array
	 ...
	 */
	function update($id = null, $fields, $tableName = null) {
		if (empty ($id)) {
	...
	function replace($fields = null, $tableName = null) {
		...
		return $this->dbo()->replace($tableName, $fields, $this->pairs($fields));
	}
...
/**
 * Thrown if you used a query the model is not shure what to return
 * 
 * @package kata_model
 */
class DatabaseUnshureWhatToReturn extends Exception {
