    ...
    foreach ($schema['cols'] as $name=>$col) {
        ...
        switch ($col['type']) {
            case 'int':
            	...
            	echo '<input type="text" onkeypress="return numericOnly(this,event);" name="data['.$name.']" value="'.$cellData.'" maxlength="'.$col['length'].'">';
            	break;
            ...
            case 'unixdate':
            	...
            	echo '<input type="hidden" name="data['.$name.']" value="'.$cellData.'"><span id="data'.$name.'" onclick="editDate(this);">'.date('d.m.Y H:i',$cellData).'</span>';
                break;
        }
        echo '</td></tr>'.chr(10);
    }
