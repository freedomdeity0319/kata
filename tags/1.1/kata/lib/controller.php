...
class Controller {
	...
	/**
	 * which helpers to use inside the view. html is default
	 * @var array
	 */
	...
	/**
	 * render the view automatically just after the controllers appropriate action was called
	 * @var boolean
	 */
	...
	function redirect($url, $status= null, $die= true) {
		...
		if($die) {
			if(DEBUG<1) {
				...
				'</head>'.
				'<body>Redirect to <a href="'.$url.'">'.$url.'</a></body></html>';
			}
			die;
		}
	}
