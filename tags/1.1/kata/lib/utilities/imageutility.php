...
class ImageUtility {
	 ...
	 * @param string $filename
	 */
	function writeImage($img, $type, $filename) {
		switch ($type) {
			case IMAGETYPE_GIF :
			...
			case IMAGETYPE_PNG :
				...
				break;
			default :
				throw new Exception('writeImage: Uknown image type');
		}
	}
 ...
 */
    function getNudity($img,$colorA=0x793B24,$colorB=0xFEC5BF)
    {
        if(!$img) return false;
        ...
        $arA['R'] = ($colorA >> 16) & 0xFF;
        $arA['G'] = ($colorA >> 8) & 0xFF;
        $arA['B'] = $colorA & 0xFF;

        $arB['R'] = ($colorB >> 16) & 0xFF;
        $arB['G'] = ($colorB >> 8) & 0xFF;
        $arB['B'] = $colorB & 0xFF;
