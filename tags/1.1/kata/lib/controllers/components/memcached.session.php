...
/**
 * session functions for php-versions that dont support memcached-sessions
 *
 * @author mnt@codeninka.de
 * @package kata
 * @ignore
 */
function mcdsess_open($save_path, $session_name) {
	return(true);
}

function mcdsess_close() {
	return(true);
}

function mcdsess_read($id) {
	$temp = $GLOBALS['mcdsess_handle']->get(SESSION_STRING.$id);
	if (false !== $temp) {
...
function mcdsess_write($id, $sess_data) {
	return $GLOBALS['mcdsess_handle']->set(SESSION_STRING.$id,serialize($sess_data),MEMCACHE_COMPRESSED,SESSION_TIMEOUT);
}

function mcdsess_destroy($id) {
	return $GLOBALS['mcdsess_handle']->delete(SESSION_STRING.$id);
}

function mcdsess_gc($maxlifetime) {
	return true;
}
...
class SessionComponent
{
	...
	private function initSession() {
		...
		if (version_compare(phpversion('memcache'),'2.1.2','<'))  {
			$GLOBALS['mcdsess_handle'] = new Memcache;
			$servers = explode(',',MEMCACHED_SERVERS);
			foreach ($servers as $server) {
				$temp = explode(':',$server);
				$GLOBALS['mcdsess_handle']->addServer($temp[0],empty($temp[1])?11211:$temp[1],true);
			}
			session_set_save_handler("mcdsess_open", "mcdsess_close", "mcdsess_read", "mcdsess_write", "mcdsess_destroy", "mcdsess_gc");
		} else {
