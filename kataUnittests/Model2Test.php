...
class Model2Test extends kataTestBaseClass {
	private $myModel= null;
	private $defaultSQL= "mysql";
	 ...
	 **/
	private function identyInsertON($table= 'idTable') {
		if ($this->defaultSQL == "mssql") {
			$this->myModel->query('SET IDENTITY_INSERT dbo.'.$table.' ON GO');
		}
	}

	private function identyInsertOFF($table= 'idTable') {
		if ($this->defaultSQL == "mssql") {
			$this->myModel->query('SET IDENTITY_INSERT dbo.'.$table.' OFF GO');
		}
	}
...
//			"none" => false,
	...
	function setup() {
		...
		if ($this->myModel === null) {
			...
			$this->sendMessage('<h1>using '.$this->defaultSQL.'</h1>');
		...
		if ($this->defaultSQL == 'mysql') {
			...
			mysql_query("CREATE TABLE tableno (id INT NOT NULL DEFAULT '0')", $link);
	...
	function testChangeConnection() {
		...
		$data= $this->myModel->find('all', array (), 'tableno');
		$this->assertTrue(isset ($data[0]['id']));
		//TODO kapier ich nich:
		$this->assertEqual($data[0]['id'], 3);
		//var_dump($data);var_dump($this->myModel->getConfig());var_dump($this->myModel->dbo());
		unset ($data);
		$this->myModel->changeConnection('conn1');
		$data= $this->myModel->find('all', array (), 'tableno');
		$this->assertTrue(isset ($data[0]['id']));
	...
	function testCreateParam() {
		...
		function testCreateExceptionDuplicate() {
			$this->myModel->query('DELETE FROM idTable');
			$this->myModel->create(array (
				"name" => "Hans",
				"Number" => 5
			), 'idTable');
		...
		function testDeleteParams() {
			...
			foreach ($this->bedingungen as $value => $bedingung) {
				if (isset ($this->bedingungExpectation[$value]["delete"])) {
					...
					$this->sendMessage(gettype($bedingung).':'.$bedingung);
