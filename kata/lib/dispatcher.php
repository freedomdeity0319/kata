...
class dispatcher {
	...
	final function dispatch($url, $routes = null) {
		...
		try {
			...
			if ((substr($this->params['action'], 0, 6) == 'cache-') && (('css' == $lowername) || ('js' == $lowername))) {
				$this->handleCachedFiles($lowername, substr($this->params['action'], 6));
			}
			...
			//TODO check if class exists, throw exception otherwise
			$c = new $classname;
			$this->controller = $c;
	...
	private function handleCachedFiles($what, $version) {
		$filename = KATATMP . 'cache' . DS . basename($what) . '.cache' . $version;
