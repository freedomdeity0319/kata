...
/**
 * generate individual tags if debugging is enabled OR pack all files into a single one, chache the file,
 * replace with a single tag that points to a .php script inside webroot/xxx/ that reads the cached file
 ...
 */
	private function joinFiles($files,$target,$tagFormat,$cacheAndMinify) {
		...
		$cacheFile = KATATMP.'cache'.DS.$target.'.cache';
		if (file_exists($cacheFile) && (time()-filemtime($cacheFile)<HOUR)) {
			return sprintf($tagFormat,$this->url('katacache/'.$target.(defined('VERSION')?'?'.VERSION:'')));
		}
		...
		return sprintf($tagFormat,$this->url('katacache/'.$target));
	}
