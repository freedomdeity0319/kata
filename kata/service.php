...
if (isset($options['h']) || isset($options['?']) || !isset($options['y'])) {
	...
	-l  <locatool-projectname> update language files from locatool
	    (you have to export the project first)
...
if (!empty($options['l'])) {
	$locaDir = ROOT . 'controllers' . DS . 'lang' . DS;
	if (file_exists($locaDir) && is_dir($locaDir)) {

		msg('Updating locafiles');
		$httpUtil = getUtil('http');
		$locaFiles = glob($locaDir . '*.php');
		foreach ($locaFiles as $file) {
			$file = basename($file);
			if ((strlen($file) != 6) || (substr($file, -4, 4) != '.php')) {
				continue;
			}
			$lang = substr($file, 0, 2);
			$txt = $httpUtil->get('http://loca.gameforge.de/out/' . $options['l'] . '/' . $lang . '.php.txt');

			if ((false !== strpos($txt, '<?')) && (false !== strpos($txt, '$messages'))) {
				file_put_contents($locaDir . $lang . '.php', $txt);
				msg($lang);
			} else {
				msg('malformed response for ' . $lang);
			}//http=200
		}//foreach
	}//locaDir
}//-l
